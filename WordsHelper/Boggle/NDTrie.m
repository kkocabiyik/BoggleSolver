...
{
	NDMutableTrie		* theTrie = (__bridge NDMutableTrie*)aContext;
...
{
	return [[self alloc] initWithTrie:anAnotherTrie];
}
...
+ (id)trieWithStrings:(NSString *)aFirstString, ...
{
	...
	theResult = [[self alloc] initWithStrings:aFirstString arguments:theArgList];
	va_end( theArgList );
	return theResult;
}
...
{
	return [[self alloc] trieWithStrings:aStrings count:aCount];
}
...
{
	[(__bridge id)anArray addObject:aString];
	return YES;
}
- (NSArray *)everyString
{
	NSMutableArray		* theResult = [NSMutableArray arrayWithCapacity:[self count]];
	forEveryStringFromNode( [self root], NULL, _addToArrayFunc, (__bridge void *)(theResult) );
	return theResult;
}
...
- (NSArray *)everyStringWithPrefix:(NSString *)aPrefix
{
	...
	forEveryStringFromNode( theNode, aPrefix, _addToArrayFunc, (__bridge void *)(theResult) );
	return theResult;
}
...
{
	BOOL	theStop = NO;
	void (^theBlock)(NSString *, BOOL *) = (__bridge void (^)(NSString *, BOOL *))aContext;
	theBlock( aString, &theStop );
	return !theStop;
}
- (void)enumerateStringsUsingBlock:(void (^)(NSString *, BOOL *))aBlock
{
	forEveryStringFromNode( [self root], NULL, enumerateFunc, (__bridge void*)aBlock );
}

- (void)enumerateStringsWithPrefix:(NSString*)aPrefix usingBlock:(void (^)(NSString * string, BOOL *stop))aBlock
{
	struct trieNode		* theNode = findNode( [self root], aPrefix, 0, NO, NULL, NULL );
	forEveryStringFromNode( theNode, aPrefix, enumerateFunc, (__bridge void*)aBlock );
}
...
{
	return [self isMemberOfClass:[NDTrie class]] ? self : [[NDTrie allocWithZone:aZone] initWithTrie:self];
}
